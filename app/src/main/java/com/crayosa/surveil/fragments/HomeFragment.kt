    ...
    override fun onCreateView(
        ...
        if(auth.currentUser != null){
            ...
            FirebaseRepository(Firebase.firestore)
                .getEnrolledRooms(auth.currentUser!!.uid)
        }

        val list = mutableListOf<ClassRoom>(
            ClassRoom("0","CSE IIIrd year", "Professional English", "Mr. Vivekanand N", emptyList()),
        ...
        binding.createRoom.setOnClickListener {
            requireView().findNavController().navigate(R.id.action_homeFragment_to_createClassRoom)
        }
        return binding.root
    }
}